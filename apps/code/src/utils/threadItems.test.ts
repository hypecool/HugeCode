...
describe("threadItems", () => {
  ...
  it("preserves local-only tool items in their original position while merging", () => {
    ...
    expect(merged.map((item) => item.id)).toEqual(["user-1", "tool-1", "assistant-1"]);
  });

  it("anchors remote-only items without pushing preserved local tool items to the end", () => {
    const remoteItems: ConversationItem[] = [
      {
        id: "user-1",
        kind: "message",
    ...
    const localItems: ConversationItem[] = [
      remoteItems[0],
      {
        ...
        kind: "tool",
        toolType: "commandExecution",
        title: "Command: git diff -- src/app.ts",
        detail: "/repo",
        status: "completed",
        output: "",
      },
    ...
    expect(merged.map((item) => item.id)).toEqual(["user-1", "tool-1", "assistant-1", "diff-1"]);
  });
  ...
  it("normalizes lifecycle tool status values to canonical casing", () => {
    const item = buildConversationItem({
      type: "plan",
      id: "plan-status-1",
      status: "in_progress",
    ...
    if (item && item.kind === "tool") {
      ...
      expect(item.status).toBe("inProgress");
      expect(item.detail).toBe("inProgress");
    }
  });
