...
function formatCollabAgentStates(value: unknown) {
  ...
  const entries = Object.entries(value as Record<string, unknown>)
    .map(([id, state]) => {
      const status = asString((state as Record<string, unknown>)?.status ?? state ?? "");
    ...
    return {
      ...
      changes: item.changes
        ? item.changes.map((change) => ({
            ...change,
            diff: isNoTruncateTool || !change.diff ? change.diff : truncateText(change.diff),
          }))
        : item.changes,
    };
  }
...
export function prepareThreadItems(items: ConversationItem[]) {
  ...
  const limited =
    normalized.length > MAX_ITEMS_PER_THREAD ? normalized.slice(-MAX_ITEMS_PER_THREAD) : normalized;
...
export function upsertItem(list: ConversationItem[], item: ConversationItem) {
  ...
  if (existing.kind === "tool" && item.kind === "tool") {
    ...
    next[index] = {
      ...
      checkpointId:
        typeof item.checkpointId === "string" ? item.checkpointId : existing.checkpointId,
      traceId: typeof item.traceId === "string" ? item.traceId : existing.traceId,
      errorClass: typeof item.errorClass === "string" ? item.errorClass : existing.errorClass,
      recovered: typeof item.recovered === "boolean" ? item.recovered : existing.recovered,
      batchId: typeof item.batchId === "string" ? item.batchId : existing.batchId,
      attempt: typeof item.attempt === "number" ? item.attempt : existing.attempt,
    };
    return next;
  }
  ...
  if (type === "mcpToolCall") {
    ...
    const argsRecord =
      item.arguments && typeof item.arguments === "object"
        ...
        : {};
    const args = Object.keys(argsRecord).length > 0 ? JSON.stringify(argsRecord, null, 2) : "";
    const durationMs =
      asNumber(item.durationMs ?? item.duration_ms) ??
      asNumber(argsRecord.durationMs ?? argsRecord.duration_ms);
    const batchId = asString(item.batchId ?? item.batch_id ?? argsRecord.batchId ?? "");
    const attempt =
      asNumber(item.attempt ?? item.attempt_count) ?? asNumber(argsRecord.attempt ?? null);
    const checkpointId = asString(
      item.checkpointId ?? item.checkpoint_id ?? argsRecord.checkpointId ?? ""
    );
    const traceId = asString(item.traceId ?? item.trace_id ?? argsRecord.traceId ?? "");
    const errorClass = asString(item.errorClass ?? item.error_class ?? argsRecord.errorClass ?? "");
    const recoveredRaw = item.recovered ?? item.recovered_state ?? argsRecord.recovered;
    const recovered =
      typeof recoveredRaw === "boolean"
        ? recoveredRaw
        : typeof recoveredRaw === "number"
          ? recoveredRaw !== 0
          : typeof recoveredRaw === "string"
            ? ["true", "1", "yes", "on", "enabled"].includes(recoveredRaw.toLowerCase())
            : null;
    const detailMeta = [
    ...
    return {
      ...
      detail: [args, detailMeta].filter(Boolean).join("\n"),
      status,
...
export function mergeThreadItems(remoteItems: ConversationItem[], localItems: ConversationItem[]) {
  ...
  const sharedIds = remoteItems.filter((item) => localIdSet.has(item.id)).map((item) => item.id);
  ...
  const merged = localItems.map((item) => {
    ...
    return remote ? chooseRicherItem(remote, item) : item;
  });

  const orderedIds = merged.map((item) => item.id);

  remoteItems.forEach((item, remoteIndex) => {
    ...
    // Keep local-only items in place while anchoring remote-only items near their
    // closest synchronized neighbors from the authoritative thread snapshot.
    for (let index = remoteIndex - 1; index >= 0; index -= 1) {
      const previousId = remoteItems[index]?.id;
      const previousResultIndex = orderedIds.indexOf(previousId);
      if (previousResultIndex >= 0) {
        insertIndex = previousResultIndex + 1;
        break;
      }
    }

    if (insertIndex === -1) {
      for (let index = remoteIndex + 1; index < remoteItems.length; index += 1) {
        const nextId = remoteItems[index]?.id;
        const nextResultIndex = orderedIds.indexOf(nextId);
        if (nextResultIndex >= 0) {
          insertIndex = nextResultIndex;
          break;
        }
      }
    }
