...
function readStoredHistory(key: string): string[] {
  ...
  const raw = readSafeLocalStorageItem(getStorageKey(key));
  if (!raw) {
    return [];
  }
  try {
...
function writeStoredHistory(key: string, value: string[]) {
  ...
  if (value.length === 0) {
    removeSafeLocalStorageItem(getStorageKey(key));
    return;
  }
  writeSafeLocalStorageItem(getStorageKey(key), JSON.stringify(value));
}
