...
import {
  ...
  isReservedGroupName,
  normalizeGroupName,
  RESERVED_WORKSPACE_GROUP_NAME,
} from "./useWorkspaces.helpers";

type UseWorkspaceGroupMutationsParams = {
...
export function useWorkspaceGroupMutations({
  ...
  workspaces,
  updateWorkspaceSettings,
}: UseWorkspaceGroupMutationsParams) {
  ...
  const createWorkspaceGroup = useCallback(
    async (name: string) => {
      ...
      if (isReservedGroupName(trimmed)) {
        throw new Error(`"${RESERVED_WORKSPACE_GROUP_NAME}" is reserved.`);
      }
