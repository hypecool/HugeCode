...
function normalizeFilePanelMode(mode: FilePanelMode): FilePanelMode {
  return mode === "atlas" ? "git" : mode;
}
...
export function useGitPanelController({
  ...
  const [gitPanelMode, setGitPanelMode] = useState<"diff" | "log" | "issues" | "prs">("diff");
  const [gitDiffViewStyle, setGitDiffViewStyle] = useState<"split" | "unified">("split");
  const [filePanelMode, setFilePanelModeState] = useState<FilePanelMode>("git");
  ...
  const diffUiVisible =
    centerMode === "diff" || (isCompact ? activeTab === "review" : gitPanelMode === "diff");
  ...
  const shouldLoadGitLog = Boolean(activeWorkspace) && (gitPanelMode === "log" || diffUiVisible);
  ...
  const activeDiffs =
    diffSource === "commit" ? gitCommitDiffs : diffSource === "pr" ? prDiffs : gitDiffs;
  ...
  const activeDiffError =
    diffSource === "commit" ? gitCommitDiffsError : diffSource === "pr" ? prDiffsError : diffError;
  ...
  const setFilePanelMode = useCallback((mode: FilePanelMode) => {
    setFilePanelModeState(normalizeFilePanelMode(mode));
  }, []);
