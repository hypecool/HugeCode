...
describe("useWorkspaceActions telemetry", () => {
  ...
  it("guards concurrent addWorkspace calls to avoid duplicate picker launches", async () => {
    let resolveWorkspace: ((value: WorkspaceInfo | null) => void) | null = null;
    const addWorkspace = vi.fn(
      () =>
        new Promise<WorkspaceInfo | null>((resolve) => {
    ...
    const firstCall = result.current.handleAddWorkspace();
    const secondCall = result.current.handleAddWorkspace();
