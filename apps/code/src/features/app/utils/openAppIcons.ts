...
export function resolveOpenAppIcon(
  target: Pick<OpenAppTarget, "id" | "kind">,
  iconById?: Record<string, string>
): string {
  if (target.kind === "finder") {
    return getKnownOpenAppIcon("finder") ?? GENERIC_FOLDER_ICON;
  }

  return getKnownOpenAppIcon(target.id) ?? iconById?.[target.id] ?? GENERIC_APP_ICON;
}
