...
export const isNarrowSettingsViewport = (): boolean => {
  ...
  try {
    const query = window.matchMedia(`(max-width: ${SETTINGS_MOBILE_BREAKPOINT_PX}px)`) as {
      matches?: unknown;
    } | null;
...
export const getOrbitStatusText = (value: OrbitActionResult, fallback: string): string => {
  if ("ok" in value) {
    if (!value.ok) {
...
export const buildShortcutDrafts = (appSettings: AppSettings): ShortcutDrafts => ({
  ...
  contextRail: appSettings.toggleGitSidebarShortcut ?? "",
