...
type SetupOverrides = {
  ...
  upsertThreadAtlasMemoryDigest?: (
    workspaceId: string,
    threadId: string,
    digest: { summary: string; updatedAt: number }
  ) => void;
...
const makeOptions = (overrides: SetupOverrides = {}) => {
  ...
  const { result } = renderHook(() =>
    useThreadItemEvents({
      ...
      upsertThreadAtlasMemoryDigest: overrides.upsertThreadAtlasMemoryDigest,
...
describe("useThreadItemEvents", () => {
  ...
  it("normalizes lifecycle item status values before building conversation items", () => {
    const { result } = makeOptions();
  ...
  it("updates long-term memory digest when context compaction completes", () => {
    const upsertThreadAtlasMemoryDigest = vi.fn();
    const { result } = makeOptions({ upsertThreadAtlasMemoryDigest });
    const item: ItemPayload = {
      type: "contextCompaction",
      id: "compact-2",
      detail: "Compaction summary digest",
      updatedAt: 4567,
    };
    ...
    expect(upsertThreadAtlasMemoryDigest).toHaveBeenCalledWith("ws-1", "thread-1", {
      summary: "Compaction summary digest",
      updatedAt: 4567,
    });
  });
